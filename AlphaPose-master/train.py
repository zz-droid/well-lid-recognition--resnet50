...
#sys.path.append('D:/AlphaPose-master/AlphaPose-master/alphapose')
...
valid_batch = 1 * max(num_gpu, 1)  # 无 GPU 时按 1 算，避免 batch 为 0
device = opt.device
# Windows 下 num_workers>0 易触发 EOFError，统一改为 0
num_workers = 0 if platform.system() == 'Windows' else opt.nThreads
if opt.sync:
...
def train(opt, train_loader, m, criterion, optimizer, writer):
    ...
    for i, (inps, labels, label_masks, _, bboxes) in enumerate(train_loader):
        if isinstance(inps, list):
            inps = [inp.to(device).requires_grad_() for inp in inps]
        else:
            inps = inps.to(device).requires_grad_()
        if isinstance(labels, list):
            labels = [label.to(device) for label in labels]
            label_masks = [label_mask.to(device) for label_mask in label_masks]
        else:
...
def validate(m, opt, heatmap_to_coord, batch_size=20):
    ...
    for inps, crop_bboxes, bboxes, img_ids, scores, imghts, imgwds in tqdm(det_loader, dynamic_ncols=True):
        if isinstance(inps, list):
            inps = [inp.to(opt.device) for inp in inps]
        else:
...
def main():
    ...
    # Model Initialize（无 CUDA 时在 CPU 上训练）
    m = preset_model(cfg)
    if num_gpu > 0:
    ...
    combined_loss = (cfg.LOSS.get('TYPE') == 'Combined')
    if combined_loss:
        criterion1 = builder.build_loss(cfg.LOSS.LOSS_1).to(device)
        criterion2 = builder.build_loss(cfg.LOSS.LOSS_2).to(device)
        criterion = [criterion1, criterion2]
    else:
        criterion = builder.build_loss(cfg.LOSS).to(device)
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=cfg.TRAIN.BATCH_SIZE * max(num_gpu, 1), shuffle=True, num_workers=num_workers)
    ...
    for i in range(cfg.TRAIN.BEGIN_EPOCH, cfg.TRAIN.END_EPOCH):
        ...
        if (i + 1) % opt.snapshot == 0:
            ...
            torch.save(model_to_save.state_dict(), './exp/{}-{}/model_{}.pth'.format(opt.exp_id, cfg.FILE_NAME, opt.epoch))
            # Prediction Test
            with torch.no_grad():
                gt_AP = validate_gt(model_to_save, opt, cfg, heatmap_to_coord)
                rcnn_AP = validate(model_to_save, opt, heatmap_to_coord)
                logger.info(f'##### Epoch {opt.epoch} | gt mAP: {gt_AP} | rcnn mAP: {rcnn_AP} #####')

        # Time to add DPG
        if i == cfg.TRAIN.DPG_MILESTONE:
            model_to_save = m.module if hasattr(m, 'module') else m
            torch.save(model_to_save.state_dict(), './exp/{}-{}/final.pth'.format(opt.exp_id, cfg.FILE_NAME))
    ...
    model_to_save = m.module if hasattr(m, 'module') else m
    torch.save(model_to_save.state_dict(), './exp/{}-{}/final_DPG.pth'.format(opt.exp_id, cfg.FILE_NAME))
